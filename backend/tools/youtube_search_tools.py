    ...
    def _run(self, keyword: str, max_results: int = 10) -> List[VideoSearchResult]:
        ...
        if not api_key:
            raise ValueError(
                ...
                "Por favor, configura la variable de entorno YOUTUBE_API_KEY con tu clave de API de YouTube."
            )
        ...
        if response.status_code == 403:
            raise ValueError(
                f"Error 403: Acceso denegado a la API de YouTube. "
                f"Verifica que tu YOUTUBE_API_KEY sea válida y tenga los permisos necesarios. "
